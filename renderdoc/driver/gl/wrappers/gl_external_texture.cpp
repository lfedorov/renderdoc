...
rdcarray<byte> WrappedOpenGL::ReadExternalTextureData(GLuint texture)
{
  ...
  switch(internal_format)
  {
    case eGL_R8: format = eGL_RED; break;
    case eGL_RG8:
      format = eGL_RG;
      size *= 2;
      break;
...
EGLImageKHR WrappedOpenGL::CreateEGLImage(GLint width, GLint height, GLenum internal_format)
{
...
#if defined(RENDERDOC_PLATFORM_ANDROID)
  //ExternalTextureResources& etr = m_ExternalTextureResources[image_index];
  ...
  switch(internal_format)
  {
    ...
    case eGL_RGB8:
      buffer_format =
          AHARDWAREBUFFER_FORMAT_R8G8B8_UNORM;    // AHARDWAREBUFFER_FORMAT_R8G8B8X8_UNORM;    // ???
      break;
    case eGL_RGBA8: buffer_format = AHARDWAREBUFFER_FORMAT_R8G8B8A8_UNORM; break;
...
void WrappedOpenGL::WriteExternalTexture2(EGLImageKHR egl_image, const byte *pixels, uint64_t size)
{
  //RDCASSERT(image_index < m_ExternalTextureResources.size());
#if defined(RENDERDOC_PLATFORM_ANDROID)
